...
const HeroSection = ({ data }) => {
  ...
  return (
    <div className="px-3 md:px-10 flex flex-col lg:flex-row items-center justify-evenly max-w-[90rem] mx-auto py-20 lg:py-12">
      <div className="flex flex-col justify-center">
        <div>
          <p className="flex flex-col items-center glowing-text3 space-y-5 mb-16 font-extrabold uppercase text-[1.7rem] md:text-6xl font-['Oswald']">
            <span className="glowing-text">{NewSlogan[0]} {NewSlogan[1]} {NewSlogan[2]}</span>
            <span className="glowing-text">{NewSlogan[3]}</span>
            <span className="glowing-text">{NewSlogan[4]} {NewSlogan[5]} {NewSlogan[6]} {NewSlogan[7]}</span>
          </p>
        </div>
